...
import GanttView from "@/app/components/GanttView";
...
export default function ProjectGanttPage() {
  ...
  const [changedTaskIds, setChangedTaskIds] = useState(new Set());
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [projectMembers, setProjectMembers] = useState([]);
  const [isSaving, setIsSaving] = useState(false);
  const [ganttViewKey, setGanttViewKey] = useState(1);
  ...
  const [viewOptions, setViewOptions] = useState({
    ...
    showSlack: false,
    colorBy: 'status', // status, priority, assignee
    zoom: 'days' // days, weeks, months
  });
  ...
  const fetchTasksAndMembers = useCallback(async () => {
    ...
    try {
      ...
      // Sort tasks by start date for better Gantt visualization
      const sortedTasks = tasksWithDateObjects.sort((a, b) => {
        const dateA = a.startDate || new Date();
        const dateB = b.startDate || new Date();
        return dateA - dateB;
      });
      ...
      setGanttViewKey((prevKey) => prevKey + 1);
  ...
  const handleGanttTasksUpdate = useCallback(
    (updatedTasksFromGantt) => {
      const updatedDisplayTasks = [...displayTasks];
      
      updatedTasksFromGantt.forEach((updatedTask) => {
  ...
  const filteredTasks = useMemo(() => {
    ...
    // Hide completed tasks if option is unchecked
  ...
  if (loading && !originalTasks.length) {
    return (
      <p className="text-center text-gray-500 py-10">Loading Gantt chart...</p>
    );
  }
  ...
  return (
    <div className="h-screen flex flex-col bg-gray-900 text-white overflow-hidden">
      <div className="flex-shrink-0 p-4 md:p-6 border-b border-slate-700/50 bg-gradient-to-r from-slate-800/80 to-slate-700/80 backdrop-blur-sm">
        <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
          <h1 className="text-2xl sm:text-3xl font-bold text-slate-100 bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">
            Gantt Chart
          </h1>
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <select
                ...
                onChange={(e) => setViewOptions(prev => ({ ...prev, zoom: e.target.value }))}
                className="px-3 py-2 bg-slate-700 border border-slate-600 rounded-lg text-white text-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20"
              >
              ...
              <select
                ...
                onChange={(e) => setViewOptions(prev => ({ ...prev, colorBy: e.target.value }))}
                className="px-3 py-2 bg-slate-700 border border-slate-600 rounded-lg text-white text-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20"
              >
      ...
      {/* Filter Panel */}
      {showFilters && (
        <div className="flex-shrink-0 bg-slate-800/50 border-b border-slate-700/50 backdrop-blur-sm">
          <div className="p-6">
              ...
              <div className="flex items-center gap-6">
                ...
                <label className="flex items-center cursor-pointer">
                  ...
                  <div className={`relative w-11 h-6 rounded-full transition-colors duration-200 ease-in-out ${
                    filters.showCompleted ? 'bg-green-600' : 'bg-slate-600'
                  }`}>
                  ...
                  <span className="ml-3 text-sm text-slate-300">Show Completed</span>
                </label>
      ...
      {error && isSaving && (
        <div className="flex-shrink-0 p-4 bg-red-900/50 border-b border-red-700 backdrop-blur-sm">
      ...
      <div className="flex-1 min-h-0">
        <GanttView
          ...
          onTasksChange={handleGanttTasksUpdate}
        />
      </div>
    </div>
  );
